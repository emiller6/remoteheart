...
%if ~exist('myocardial_infarction_model/ECGData/data', 'dir')
helperCreateECGDirectories(data, labels, 'myocardial_infarction_model/ECGData', 'data');
%end
helperPlotReps(data, labels)

%Time-Frequency Representations
helperCreateRGBfromTF(data, labels, 'myocardial_infarction_model/ECGData', 'data');

%Separate Training and Testing Data
all_imgs = imageDatastore('myocardial_infarction_model/ECGData/data','IncludeSubfolders',true,'LabelSource','foldernames');
rng default
...
%%Helper Functions
function helperCreateECGDirectories(data, labels,parentFolder,dataFolder)
%creates data directory and then makes subdirectories for each class of ecg signal
  ...
  folderLabels = unique(labels);
  for i = 1:numel(folderLabels)
      mkdir(fullfile(rootFolder,localFolder,char(folderLabels(i))));
  end
end

function helperPlotReps(data, labels)
% Plots the first thousand samples of representative of each class
  folderLabels = unique(labels);

  for k=1:3
      ecgType = folderLabels{k};
      ind = find(ismember(labels,ecgType));
      subplot(3,1,k)
      plot(data(ind(1),1:1000));
      grid on
      title(ecgType)
  end
end

function helperCreateRGBfromTF(data,labels,parentFolder,childFolder)
